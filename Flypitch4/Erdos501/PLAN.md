...
| S7 | **resolved by restating over the random algebra** (`Main.lean`): the assertion is proved for `V (randomAlgebra RandomIndex)` (𝔠⁺ random reals over the ground model, no collapse); the literal `𝔹_col_random` version would need the theory of names in the product `Col × Random` and is not pursued (`𝔹_col_random` is only defined, `ColRandom.lean`; the formerly `sorry`-stated assertion `erdos501_of_col_random` was removed) | — |
