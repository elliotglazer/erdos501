...
| Statement shapes | `Challenge.lean`, `ChallengeFlypitch.lean` | `google-deepmind/formal-conjectures` `FormalConjectures/ErdosProblems/501.lean` at `e7f4b0e` (Apache 2.0); audited 2026‑08‑16 (`docs/audits/…formal-conjectures…`) | v4.27.0 / a3a10db | statements re-typed here, no code copied | n/a |
| Mathlib-only challenge and the bridge to Mathlib's first-order logic (`Challenge.lean` Part A: `L`, `ZFC`, `Erdos501`, `zfsetStructure`; `Solution.lean`; `Erdos501/FOL/{Statement,Translate,FolLemmas,Collection,Axioms,Sentence,Independence}.lean`; `scripts/sync-statement.py`; `formalization.yaml`) | as listed | session 2026‑08‑19 (Palomar conformance), written directly at the unified pin; `Challenge.lean` Part A mirrors `Flypitch4/Zfc.lean` (axioms) and `Flypitch4/Erdos501/Sentence.lean` (sentence) combinator by combinator, in Mathlib's `ModelTheory`; the previous Flypitch-based pair was renamed `ChallengeFlypitch.lean`/`SolutionFlypitch.lean`/`comparator-flypitch.json` | v4.34.0-rc1 / 355bc1e | builds; axioms standard (`docs/audits/2026-08-19-axiom-audit-targets-355bc1e.txt`); both comparator configurations pass | n/a |
| Paper | `docs/paper/erdos501_random_profiles_rev10.pdf` | E. Glazer, "Erdős Problem 501 after adding ω₂ random reals", draft rev10 (2026‑08‑16); the earlier draft rev09 was removed from the repository on 2026‑08‑17 (its audit note remains in `docs/audits/`) | — | audited (`docs/audits/`) | — |
