...
# Erdős Problem #501 — comparator solution (Flypitch's terms)

The untrusted half of the second comparator challenge: the statements of
`ChallengeFlypitch.lean`, repeated verbatim, each proved by delegation to the
`Erdos501` development.  Only the statements are compared by the comparator;
this file may import anything.

All seven targets are proved; `comparator-flypitch.json` lists them all.
-/
