...
# Erdős Problem #501 — comparator challenge in Flypitch's terms (trusted statements)

This file is the *trusted* half of the second comparator challenge of this
repository (`comparator-flypitch.json`; the first, Palomar-conformant one is
`Challenge.lean` / `Solution.lean` / `comparator.json`, which imports Mathlib only
and states independence in Mathlib's `ModelTheory`).  It states, with `sorry`,
exactly what the accompanying `SolutionFlypitch.lean` must prove, in the
proof-theoretic terms of the vendored Flypitch development
(`independent ZFC Erdos501_f`, i.e. `¬ (ZFC ⊢ₛ' Erdos501_f) ∧ ¬ (ZFC ⊢ₛ' ∼Erdos501_f)`).
Everything that this file imports is part of the trusted base and should be
audited by a human:
...
This is a theorem of `ZFC`, so it holds in Lean outright; the hypothesis `CH` is
a genuine hypothesis of the theorem.  (Hechler's construction in fact yields
